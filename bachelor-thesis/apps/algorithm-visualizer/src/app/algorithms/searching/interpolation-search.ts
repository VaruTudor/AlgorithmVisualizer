  ...
  while (left <= right && target >= array[left] && target <= array[right]) {
    if (left === right) {
      if (array[left] === target) {
        animations.push(new FoundAnimation(left));
        return animations;
      }
      break;
    }
    ...
    if (previous !== -1 && current !== 0)
      animations.push(new CurrentChangeAnimation(previous, current));
    else animations.push(new CurrentChangeAnimation(current, current));
    if (array[current] == target) {
      animations.push(new FoundAnimation(current));
      return animations;
    } else if (array[current] < target) {
      animations.push(new BetterMatchAnimation(current + 1, left));
      left = current + 1;
    } else {
      animations.push(new BetterMatchAnimation(current - 1, right));
      right = current - 1;
    }
  }
